...
def trainer(args):
    ...
    trajgenerator = TrajGeneratorNetwork(num_location=num_loc, location_embedding=args.num_hidden, maxi=maxi,
                                         num_head=args.TrajGenerator_heads,
                                         lstm_hidden=args.num_hidden, device=device,
                                         TrajGenerator_Translayers=args.TrajGenerator_Translayers,
                                         TrajGenerator_LSTMlayers=args.TrajGenerator_LSTMlayers,
                                         input_len=args.length).double().to(device)

    traj_diffusion = TrajectoryDiffusion(model=trajgenerator, maxi=maxi, lab=2, linear_start=0.00085, linear_end=0.0120,
                                         full_n_steps=1000).double().to(device)
    ...
    for e in range(args.epoch):
        ...
        if e != 0 and e % args.eval_epoch == 0:
            with torch.no_grad():
                ...
                dis_jsd, total_jsd, o_jsd, d_jsd, ratio = metrics.js_traj(real_traj=total_x,
                                                                          sythsize_traj=trajs_generated)
                print('JSD', dis_jsd, total_jsd, o_jsd, d_jsd, ratio)

            torch.save(traj_diffusion,
                       '../TrainedModels/trajGDM_' + args.dataset + '_' + str(float(np.mean(train_loss))) + '.pkl')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--TrajGenerator_LSTMlayers', default=2, type=int)
