...
class TrajectoryDiffusion(torch.nn.Module):  # DiffusionSampler
    def __init__(self, model, maxi, lab=2, linear_start: float = 0.00085, linear_end: float = 0.0120,
                 full_n_steps=1000):
        super().__init__()
        self.model = model
        self.maxi = maxi
        ...
        beta = torch.linspace(linear_start ** 0.5, linear_end ** 0.5, self.full_n_steps, dtype=torch.float64,
                              device=self.model.get_device) ** 2
    ...
    def diffusion_process(self, x0: torch.Tensor, index: torch.Tensor, noise: Optional[torch.Tensor] = None):
        ...
        return self.sqrt_alpha_bar[index].view(x0.shape[0], 1, 1) * x0 + self.sqrt_1m_alpha_bar[index].view(x0.shape[0],
                                                                                                            1,
                                                                                                            1) * noise
    ...
    def generation_training(self, x: torch.Tensor, noise: Optional[torch.Tensor] = None):
        ...
        t = torch.randint(low=0, high=self.full_n_steps, size=(batch_size,), device=self.model.get_device,
                          dtype=torch.long)  # need to revise when
        ...
        discrete_loss = torch.nn.functional.cross_entropy(current_predict_x0.view(-1, self.model.num_location),
                                                          x.view(-1, ), label_smoothing=0.0,
                                                          reduction='sum')  # *256 this leads to view
    ...
    def prediction(self, x, predict_len, ddim=True, ddim_step=50, ddim_eta=0.9, std=0):
        ...
        pred = torch.zeros((x.shape[0], predict_len, cond.shape[2]), device=self.model.get_device,
                           dtype=torch.double) + std
        ...
        if ddim:
            ...
            self.ddim_sigma = (ddim_eta * ((1 - self.ddim_alpha_prev) / (1 - self.ddim_alpha) * (
    ...
    def reconstruction(self, x1, x2, predict_len=1, ddim=True, ddim_step=50, ddim_eta=0., std=0):
        ...
        pred = torch.zeros((x1.shape[0], predict_len, cond2.shape[2]), device=self.model.get_device,
                           dtype=torch.double) + std
