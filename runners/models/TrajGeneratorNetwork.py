...
class TrajGeneratorNetwork(torch.nn.Module):
    def __init__(self, num_location, location_embedding, lstm_hidden, maxi, device=torch.device("cuda:1"), input_len=12,
                 num_head=2, TrajEncoder_layers=1, TrajGenerator_LSTMlayers=1, TrajGenerator_Translayers=1,
                 TrajDecoder_layers=1):
        ...
        self.pos_encoding1 = PositionalEncoding(num_features=location_embedding, dropout=0.1,
                                                max_len=self.input_len * 2)
        ...
        self.TrajEncoder_lstm = torch.nn.LSTM(input_size=self.loc_size, hidden_size=location_embedding,
                                              num_layers=TrajEncoder_layers, bidirectional=False, batch_first=True)
        ...
        self.TrajGenerator_encoderLSTM = torch.nn.LSTM(input_size=location_embedding, hidden_size=lstm_hidden,
                                                       num_layers=TrajGenerator_LSTMlayers, bidirectional=False,
                                                       batch_first=True)  # 2

        self.TrajGenerator_encoder = torch.nn.TransformerEncoder(
            torch.nn.TransformerEncoderLayer(d_model=location_embedding, nhead=self.num_head,
                                             dim_feedforward=lstm_hidden, dropout=0.1,
                                             activation='relu', batch_first=True), num_layers=TrajGenerator_Translayers)

        self.TrajGenerator_decoder = torch.nn.TransformerDecoder(
            torch.nn.TransformerDecoderLayer(d_model=location_embedding, nhead=self.num_head,
                                             dim_feedforward=lstm_hidden, dropout=0.1,
                                             activation='relu', batch_first=True), num_layers=TrajGenerator_Translayers)

        self.TrajGenerator_decoderLSTM = torch.nn.LSTM(input_size=location_embedding, hidden_size=lstm_hidden,
                                                       num_layers=1, bidirectional=False, batch_first=True)  # 2

        self.TrajDecoder_encoder = torch.nn.TransformerEncoder(
            torch.nn.TransformerEncoderLayer(d_model=location_embedding, nhead=self.num_head,
                                             dim_feedforward=lstm_hidden, dropout=0.1,
                                             activation='relu', batch_first=True), num_layers=TrajDecoder_layers)

        self.TrajDecoder_decoder = torch.nn.TransformerDecoder(
            torch.nn.TransformerDecoderLayer(d_model=location_embedding, nhead=self.num_head,
                                             dim_feedforward=lstm_hidden, dropout=0.1,
                                             activation='relu', batch_first=True), num_layers=TrajDecoder_layers)
    ...
    def TrajDecoder(self, hidden_repr):
        ...
        decoder_input = self.location_embed(
            torch.ones(size=(hidden_repr.shape[0], hidden_repr.shape[1]), device=hidden_repr.device, dtype=torch.long))
